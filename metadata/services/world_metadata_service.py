...
class WorldMetadataService:
    ...
    def __init__(self, world_metadata_repository: WorldMetadataRepository):
        
        self.world_metadata_repository = world_metadata_repository


    async def get_all_worlds(self) -> list[World]:
        
        return await self.world_metadata_repository.get_all_worlds()


    async def add_world(self, world_create_request: WorldCreateRequest) -> World:

        return await self.world_metadata_repository.add_world(
    ...
    async def update_world(self, world_update_request: WorldUpdateRequest, id: uuid.UUID) -> World:
        ...
        return await self.world_metadata_repository.update_world(world_update_request, id)
    

    async def delete_world_by_id(self, id: uuid.UUID) -> None:
        await self.world_metadata_repository.delete_world_by_id(id)
